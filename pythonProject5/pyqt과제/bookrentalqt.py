...
class Rental_page(QWidget, form_widget):
    def __init__(self):
        ...
        self.YH_main_2.setColumnWidth(2, 300)
        self.YH_main_3.setColumnWidth(0, 40)
        self.YH_main_3.setColumnWidth(1, 80)
        self.YH_main_3.setColumnWidth(2, 300)
    ...
    def real_return(self):
        ...
        for i in self.bookrental:  # bookrental list 확인
            if self.member_return_list[self.row] == i:  # 전체대여리스트와 가입자의 대여리스트가 같다면
                del (self.bookrental[check_v])  # 전체 대여리스트에서 그 줄을 빼준다
                print(self.bookrental)
            check_v += 1
        self.real_return_list.append(self.member_return_list[self.row])
        del (self.member_return_list[self.row])
        # 대여 반납 현황판 업데이트
        ...
        for s in self.real_return_list:
            ...
            self.YH_main_3.setItem(Row, 5, QTableWidgetItem(s[10]))
            Row += 1
        print(f'{self.member_return_list}')
